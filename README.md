...
> [!Note] Obsidian CLI（前置条件 / Prerequisite）
> 这个 Hook 用的是 **Obsidian CLI**（v1.12+ 内置）的 `obsidian open` 命令。理论上 `obsidian://open` URI scheme 也能实现同样效果，但 CLI 更简洁——尤其 vault 或文件名含中文时，URI 需要手动 URL encode（`%E5%89%AF%E6%9C%AC`），CLI 直接用原始路径就行。
>
...
> [!Warning] 多 Vault 限制 / Multi-Vault Limitation
> **Obsidian CLI 只能操作当前活跃连接的 vault**（通常是最后打开/聚焦的那个）。
>
> 如果你同时开着多个 vault：
> - Hook **只能打开当前活跃 vault 里的文件**
> - 文件在非活跃 vault 时会被静默跳过（日志中可见）
> - 同名文件（不同目录）可能打开错误的文件，因为 `file=` 按文件名匹配
>
> **如果你只用一个 vault，完全没有这个问题。**
>
> 解决方案（二选一）：
> 1. **写死 vault 名**：在脚本底部取消 `vault=` 那行的注释，强制指定 vault
> 2. **不用管**：接受限制，非活跃 vault 的文件不自动打开
>
> Obsidian CLI connects to one vault at a time (usually the last active one). If you run multiple vaults simultaneously, the hook can only open files in the currently active vault. If you use a single vault, this is a non-issue.
